...
TEST(Lexer, LexesOperators) {
	...
	std::vector<TokenType> expects = {
		...
		COMMA, DOT, OPEN_BRACKET, CLOSE_BRACKET, EXLUSIVE_RANGE, INCLUSIVE_RANGE,
		COLON, ARROW, TILDE
	};
...
TEST(Lexer, ClumpedOperators) {
	...
	std::vector<TokenType> expects = {
		...
		SHIFT_LEFT_ASSIGN, ASSIGN, EXLUSIVE_RANGE, DOT, NEWLINE, NEWLINE, NOT
	};
...
TEST(Lexer, LexesIdentifiers) {
	...
	ss << "try catch finally throw of as property";
